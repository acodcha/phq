...
TEST(VolumetricThermalExpansionCoefficient, ArithmeticOperatorAddition) {
  EXPECT_EQ(VolumetricThermalExpansionCoefficient(
                1.0, Unit::ThermalExpansion::PerKelvin)
                + VolumetricThermalExpansionCoefficient(
                    2.0, Unit::ThermalExpansion::PerKelvin),
            VolumetricThermalExpansionCoefficient(
...
TEST(VolumetricThermalExpansionCoefficient, AssignmentOperatorAddition) {
  VolumetricThermalExpansionCoefficient quantity{
      1.0, Unit::ThermalExpansion::PerKelvin};
  quantity += VolumetricThermalExpansionCoefficient(
      2.0, Unit::ThermalExpansion::PerKelvin);
  EXPECT_EQ(quantity, VolumetricThermalExpansionCoefficient(
                          3.0, Unit::ThermalExpansion::PerKelvin));
}
...
TEST(VolumetricThermalExpansionCoefficient, ComparisonOperators) {
  const VolumetricThermalExpansionCoefficient first{
      1.11, Unit::ThermalExpansion::PerKelvin};
  const VolumetricThermalExpansionCoefficient second{
      2.22, Unit::ThermalExpansion::PerKelvin};
...
TEST(VolumetricThermalExpansionCoefficient, CopyAssignmentOperator) {
  ...
  VolumetricThermalExpansionCoefficient second =
      VolumetricThermalExpansionCoefficient::Zero();
  second = first;
...
TEST(VolumetricThermalExpansionCoefficient, Create) {
  constexpr VolumetricThermalExpansionCoefficient quantity =
      VolumetricThermalExpansionCoefficient::Create<
          Unit::ThermalExpansion::PerKelvin>(1.11);
  EXPECT_EQ(quantity, VolumetricThermalExpansionCoefficient(
                          1.11, Unit::ThermalExpansion::PerKelvin));
}
...
TEST(VolumetricThermalExpansionCoefficient, Hash) {
  const VolumetricThermalExpansionCoefficient first{
      1.11, Unit::ThermalExpansion::PerRankine};
  const VolumetricThermalExpansionCoefficient second{
      1.110001, Unit::ThermalExpansion::PerRankine};
  const VolumetricThermalExpansionCoefficient third{
      -1.11, Unit::ThermalExpansion::PerRankine};
...
TEST(VolumetricThermalExpansionCoefficient, JSON) {
  ...
  EXPECT_EQ(VolumetricThermalExpansionCoefficient(
                -2.22, Unit::ThermalExpansion::PerRankine)
...
TEST(VolumetricThermalExpansionCoefficient, MoveAssignmentOperator) {
  ...
  EXPECT_EQ(second, VolumetricThermalExpansionCoefficient(
                        1.11, Unit::ThermalExpansion::PerKelvin));
}
...
TEST(VolumetricThermalExpansionCoefficient, MoveConstructor) {
  VolumetricThermalExpansionCoefficient first{
      1.11, Unit::ThermalExpansion::PerKelvin};
  VolumetricThermalExpansionCoefficient second{std::move(first)};
  EXPECT_EQ(second, VolumetricThermalExpansionCoefficient(
                        1.11, Unit::ThermalExpansion::PerKelvin));
}
...
TEST(VolumetricThermalExpansionCoefficient, StaticValue) {
  constexpr VolumetricThermalExpansionCoefficient quantity =
      VolumetricThermalExpansionCoefficient::Create<
          Unit::ThermalExpansion::PerRankine>(1.11);
  constexpr double value =
      quantity.StaticValue<Unit::ThermalExpansion::PerRankine>();
  EXPECT_EQ(value, 1.11);
}

TEST(VolumetricThermalExpansionCoefficient, Stream) {
  std::ostringstream stream;
  stream << VolumetricThermalExpansionCoefficient(
      1.11, Unit::ThermalExpansion::PerKelvin);
  EXPECT_EQ(stream.str(), VolumetricThermalExpansionCoefficient(
                              1.11, Unit::ThermalExpansion::PerKelvin)
                              .Print());
}

TEST(VolumetricThermalExpansionCoefficient, Unit) {
  EXPECT_EQ(VolumetricThermalExpansionCoefficient::Unit(),
            Standard<Unit::ThermalExpansion>);
}
...
TEST(VolumetricThermalExpansionCoefficient, Zero) {
  EXPECT_EQ(VolumetricThermalExpansionCoefficient::Zero(),
            VolumetricThermalExpansionCoefficient(
                0.0, Unit::ThermalExpansion::PerKelvin));
}
