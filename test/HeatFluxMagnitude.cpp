...
TEST(HeatFluxMagnitude, AssignmentOperatorAddition) {
  ...
  EXPECT_EQ(
      quantity, HeatFluxMagnitude(3.0, Unit::EnergyFlux::WattPerSquareMetre));
}
...
TEST(HeatFluxMagnitude, AssignmentOperatorDivision) {
  ...
  quantity /= 2.0;
  EXPECT_EQ(
      quantity, HeatFluxMagnitude(4.0, Unit::EnergyFlux::WattPerSquareMetre));
}
...
TEST(HeatFluxMagnitude, AssignmentOperatorMultiplication) {
  ...
  quantity *= 2.0;
  EXPECT_EQ(
      quantity, HeatFluxMagnitude(8.0, Unit::EnergyFlux::WattPerSquareMetre));
}
...
TEST(HeatFluxMagnitude, AssignmentOperatorSubtraction) {
  ...
  quantity -= HeatFluxMagnitude(2.0, Unit::EnergyFlux::WattPerSquareMetre);
  EXPECT_EQ(
      quantity, HeatFluxMagnitude(1.0, Unit::EnergyFlux::WattPerSquareMetre));
}
...
TEST(HeatFluxMagnitude, Create) {
  ...
  EXPECT_EQ(
      quantity, HeatFluxMagnitude(1.11, Unit::EnergyFlux::WattPerSquareMetre));
}
...
TEST(HeatFluxMagnitude, Dimensions) {
  EXPECT_EQ(
      HeatFluxMagnitude::Dimensions(), RelatedDimensions<Unit::EnergyFlux>);
}

TEST(HeatFluxMagnitude, Hash) {
  const HeatFluxMagnitude first{
      1.11, Unit::EnergyFlux::NanowattPerSquareMillimetre};
  const HeatFluxMagnitude second{
      1.110001, Unit::EnergyFlux::NanowattPerSquareMillimetre};
  const HeatFluxMagnitude third{
      -1.11, Unit::EnergyFlux::NanowattPerSquareMillimetre};
...
TEST(HeatFluxMagnitude, JSON) {
  EXPECT_EQ(
      HeatFluxMagnitude(1.11, Unit::EnergyFlux::WattPerSquareMetre).JSON(),
      "{\"value\":1.110000000000000,\"unit\":\"W/m^2\"}");
  EXPECT_EQ(
      HeatFluxMagnitude(-2.22, Unit::EnergyFlux::NanowattPerSquareMillimetre)
...
TEST(HeatFluxMagnitude, MoveAssignmentOperator) {
  ...
  EXPECT_EQ(
      second, HeatFluxMagnitude(1.11, Unit::EnergyFlux::WattPerSquareMetre));
}

TEST(HeatFluxMagnitude, MoveConstructor) {
  HeatFluxMagnitude first{1.11, Unit::EnergyFlux::WattPerSquareMetre};
  HeatFluxMagnitude second{std::move(first)};
  EXPECT_EQ(
      second, HeatFluxMagnitude(1.11, Unit::EnergyFlux::WattPerSquareMetre));
}
...
TEST(HeatFluxMagnitude, Print) {
  EXPECT_EQ(
      HeatFluxMagnitude(1.11, Unit::EnergyFlux::WattPerSquareMetre).Print(),
...
TEST(HeatFluxMagnitude, StandardConstructor) {
  EXPECT_NO_THROW(
      HeatFluxMagnitude(1.11, Unit::EnergyFlux::NanowattPerSquareMillimetre));
}

TEST(HeatFluxMagnitude, StaticValue) {
  constexpr HeatFluxMagnitude quantity =
      HeatFluxMagnitude::Create<Unit::EnergyFlux::NanowattPerSquareMillimetre>(
          2.0);
...
TEST(HeatFluxMagnitude, Value) {
  EXPECT_EQ(
      HeatFluxMagnitude(1.11, Unit::EnergyFlux::WattPerSquareMetre).Value(),
      1.11);
  EXPECT_EQ(
      HeatFluxMagnitude(2.0, Unit::EnergyFlux::NanowattPerSquareMillimetre)
...
TEST(HeatFluxMagnitude, YAML) {
  EXPECT_EQ(
      HeatFluxMagnitude(1.11, Unit::EnergyFlux::WattPerSquareMetre).YAML(),
      "{value:1.110000000000000,unit:\"W/m^2\"}");
  EXPECT_EQ(
      HeatFluxMagnitude(-2.22, Unit::EnergyFlux::NanowattPerSquareMillimetre)
...
TEST(HeatFluxMagnitude, Zero) {
  EXPECT_EQ(HeatFluxMagnitude::Zero(),
            HeatFluxMagnitude(0.0, Unit::EnergyFlux::WattPerSquareMetre));
}
