...
// physical quantity is composed only of a value where the value is a tensor of rank two and
// dimension three. The tensor may be non-symmetric. Such a physical quantity has no unit of measure
...
protected:
  ...
  explicit constexpr DimensionlessDyadQuantity(const Value::Dyad& value) : value_(value) {}
  ...
  // one.
  constexpr DimensionlessDyadQuantity(const DimensionlessDyadQuantity& other) = default;

  // Move constructor. Constructs a dimensionless dyadic tensor physical quantity by moving another
  // one.
  constexpr DimensionlessDyadQuantity(DimensionlessDyadQuantity&& other) noexcept = default;

  // Copy assignment operator. Assigns this dimensionless dyadic tensor physical quantity by copying
  // another one.
  constexpr DimensionlessDyadQuantity& operator=(const DimensionlessDyadQuantity& other) = default;
...
inline std::ostream& operator<<(std::ostream& stream, const DimensionlessDyadQuantity& quantity) {
...
struct hash<PhQ::DimensionlessDyadQuantity> {
  inline size_t operator()(const PhQ::DimensionlessDyadQuantity& quantity) const {
