...
// Heat capacity ratio, also known as ratio of specific heats, adiabatic index,
// or Laplace's coefficient. The heat capacity ratio is the ratio of the
// isobaric heat capacity of a material to its isochoric heat capacity.
class HeatCapacityRatio : public DimensionlessScalarQuantity {
public:
  // Default constructor. Constructs a heat capacity ratio ratio with an
  // uninitialized value.
  HeatCapacityRatio() = default;

  // Constructor. Constructs a heat capacity ratio with a given value.
  explicit constexpr HeatCapacityRatio(const double value)
  ...
  // the definition of the heat capacity ratio.
  constexpr HeatCapacityRatio(
      const SpecificGasConstant& specific_gas_constant,
      const SpecificIsochoricHeatCapacity& specific_isochoric_heat_capacity);

  // Constructor. Constructs a heat capacity ratio from a given specific
  ...
  // Constructor. Constructs a heat capacity ratio from a given isobaric heat
  ...
  // Copy assignment operator. Assigns this heat capacity ratio by copying
  // another one.
  constexpr HeatCapacityRatio& operator=(
      const HeatCapacityRatio& other) = default;

  // Move assignment operator. Assigns this heat capacity ratio by moving
  // another one.
  constexpr HeatCapacityRatio& operator=(
      HeatCapacityRatio&& other) noexcept = default;
  ...
  constexpr double operator/(
      const HeatCapacityRatio& heat_capacity_ratio) const noexcept {
...
struct hash<PhQ::HeatCapacityRatio> {
  inline size_t operator()(
      const PhQ::HeatCapacityRatio& heat_capacity_ratio) const {
