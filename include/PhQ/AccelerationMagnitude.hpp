...
public:
  ...
  // Constructor. Constructs an acceleration magnitude from a given speed and time using the
  // definition of acceleration.
  constexpr AccelerationMagnitude(const Speed& speed, const Time& time)
    : AccelerationMagnitude(speed.Value() / time.Value()) {}

  // Constructor. Constructs an acceleration magnitude from a given speed and frequency using the
  // definition of acceleration.
  constexpr AccelerationMagnitude(const Speed& speed, const Frequency& frequency)
  ...
  // Copy constructor. Constructs an acceleration magnitude by copying another one.
  constexpr AccelerationMagnitude(const AccelerationMagnitude& other) = default;

  // Move constructor. Constructs an acceleration magnitude by moving another one.
  constexpr AccelerationMagnitude(AccelerationMagnitude&& other) noexcept = default;

  // Copy assignment operator. Assigns this acceleration magnitude by copying another one.
  constexpr AccelerationMagnitude& operator=(const AccelerationMagnitude& other) = default;

  // Move assignment operator. Assigns this acceleration magnitude by moving another one.
  constexpr AccelerationMagnitude& operator=(AccelerationMagnitude&& other) noexcept = default;
  ...
  // Statically creates an acceleration magnitude with a given value expressed in a given
  // acceleration unit.
  ...
  constexpr double operator/(const AccelerationMagnitude& acceleration_magnitude) const noexcept {
...
inline constexpr bool operator==(
    const AccelerationMagnitude& left, const AccelerationMagnitude& right) noexcept {
...
inline constexpr Time::Time(const Speed& speed, const AccelerationMagnitude& acceleration_magnitude)
...
inline constexpr Speed::Speed(const AccelerationMagnitude& acceleration_magnitude, const Time& time)
  : Speed(acceleration_magnitude.Value() * time.Value()) {}

inline constexpr Speed::Speed(
    const AccelerationMagnitude& acceleration_magnitude, const Frequency& frequency)
  : Speed(acceleration_magnitude.Value() / frequency.Value()) {}

inline constexpr AccelerationMagnitude Frequency::operator*(const Speed& speed) const {
  return {speed, *this};
}

inline constexpr Speed Time::operator*(const AccelerationMagnitude& acceleration_magnitude) const {
  return {acceleration_magnitude, *this};
}

inline constexpr AccelerationMagnitude Speed::operator*(const Frequency& frequency) const {
  return {*this, frequency};
}

inline constexpr AccelerationMagnitude Speed::operator/(const Time& time) const {
  return {*this, time};
}

inline constexpr Time Speed::operator/(const AccelerationMagnitude& acceleration_magnitude) const {
...
struct hash<PhQ::AccelerationMagnitude> {
  inline size_t operator()(const PhQ::AccelerationMagnitude& acceleration_magnitude) const {
